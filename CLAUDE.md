...
- **Importador de caminantes desde Excel** (botón "Importar caminantes (Excel)" dentro de Inscripciones, en la ficha de cada retiro): a diferencia del de servidores, este también los **inscribe como caminantes en ESE retiro concreto** (usa `Store.inscribir` + `actualizarInscripcion`, empareja por DNI/email igual que el de servidores). Rellena directamente los campos de Palancas: 2 contactos de apoyo (con email), quién invitó (con teléfono y email), y sobre todo la pregunta "¿vienes con un amigo/familiar?" del formulario alimenta `mesaConoceA` — es la misma señal que usa `sugerirAsignacionMesas` para no sentarlos juntos, así que ya viene resuelta desde la importación. También trae `estado_civil` (campo nuevo en `contactos`) y si sus familiares van a la misa del domingo (`inscripciones.familiares_domingo`).
- **Habitaciones** (`habitaciones`, `habitacion_ocupantes`): asignación de caminantes/servidores a habitaciones (capacidad 1/2/3) dentro de cada retiro. Caminantes y servidores nunca comparten habitación entre sí (papel obligatorio en la tabla). `Store.sugerirAsignacionHabitaciones(retiroId)` es una heurística de mejor esfuerzo (no óptima): primero empareja peticiones explícitas de compañero (`contactos.companeroPreferido`, texto libre comparado por nombre), luego agrupa por localidad evitando edades muy parecidas y malas combinaciones de ronquidos (`contactos.ronca` / `duermeConRoncador`, también texto libre). Solo asigna a quien todavía no tenga habitación — nunca mueve a alguien ya colocado a mano, así que sirve como punto de partida editable, no como decisión final.
- **Equipo de Palancas** (`retiro_palancas_equipo`): un responsable + varios ayudantes por retiro, elegidos entre los servidores ya inscritos en ese retiro concreto (único responsable por retiro, forzado con un índice único parcial). Los datos de Palancas de cada caminante (contactos de apoyo, quién le invitó al retiro, transporte, mesa) viven en `inscripciones` con prefijo `palancas_*`, porque son propios de ESE retiro, no del contacto en general — se editan desde el botón "🕊️ Palancas" en la fila de cada caminante inscrito. También incluye reparto de llamadas (`palancas_asignado_a`, `palancas_contactado`) y un botón para enviar por WhatsApp a cada miembro su lista de caminantes asignados.
- **Equipo de Administración** (`retiro_administracion_equipo`): como Palancas pero admite **hasta 2 responsables** (no solo uno) + ayudantes, que acompañan a los caminantes a sus habitaciones. La **Lista de Administración** (dentro de la ficha de cada retiro) es la hoja de control del día de llegada: nombre, teléfono, check de "ha llegado" (`inscripciones.llegado`), pago y habitación asignada — exportable a CSV o directamente imprimible.
- **Formas de pago** (`formas_pago`, ampliable desde Ajustes) e **importe pagado por inscripción** (`inscripciones.importe_pagado`): junto al simple check "Pagado" (marca manual rápida), se puede anotar el importe exacto pagado, comparado contra `retiros.precio` para calcular cuánto debe cada uno (se ve en Inscripciones, Tesorería y la Lista de Administración).
- **Mesas** (`mesas`, `mesa_caminantes`): grupos de 5-6 personas por retiro — líder + colíder de mesa (servidores ya inscritos en ese retiro, de cualquier zona si el retiro es combinado) + 3-4 caminantes. Reglas de `Store.sugerirAsignacionMesas`: (1) dura — si dos caminantes se conocen (`inscripciones.mesa_conoce_a`, texto libre editado desde la ficha de Palancas de cada uno), nunca se sientan juntos; (2) fuerte pero no absoluta — los caminantes de la misma zona van juntos (importante en retiros combinados entre zonas, ej. Elche + Benidorm), solo se mezclan zonas en una mesa si no queda otra con hueco. Como Habitaciones, solo asigna a quien todavía no tenga mesa.
- **Retiros combinados entre zonas** (ej. Benidorm + Elche): el desplegable "+ Añadir inscrito" de cada retiro muestra primero la gente de su propia zona y, en un grupo aparte, el resto de zonas — para estos casos excepcionales sin tener que crear contactos duplicados. Líder/colíder de mesa y todo lo demás funciona igual, porque todo se organiza por retiro, no por zona.
- **Equipo de Cocina** (`retiro_cocina_equipo`, mismo patrón que Palancas: un responsable + varios ayudantes por retiro): organizan comidas, cenas y descansos. Las **alergias** (`contactos.alergias`, campo visible y destacado en la ficha de contacto, no escondido en "Más datos" por ser información de seguridad) alimentan la **Lista de alergias** de cada retiro — caminantes y servidores inscritos que tengan alguna anotada, imprimible para el equipo de Cocina.
- **Responsables de tarea puntual** (`retiro_tarea_responsables`, genérico: `retiro_id` + `tarea` + `contacto_id`): para tareas de un solo responsable que no necesitan un equipo entero (de momento solo "etiquetas", pero pensado para reutilizarse en futuras tareas simples sin crear una tabla nueva cada vez).
- **Etiquetas para colgadores**: checklist por inscrito (`inscripciones.etiqueta_impresa`) + responsable (tabla genérica de arriba) + botón que abre una ventana lista para imprimir con una etiqueta por caminante/servidor (nombre, papel, nombre del retiro), tamaño pensado para colgador.
- **Conflicto líder/colíder de mesa vs. otros equipos**: al asignar a alguien como líder/colíder de mesa (o viceversa, a Palancas/Administración/Cocina), la app avisa con un `confirm()` si esa persona ya tiene otra tarea en el mismo retiro — el líder/colíder de mesa debe estar centrado solo en sus caminantes, así que se recomienda asignarlos primero, antes que el resto de equipos (`App.otrasTareasEnRetiro` / `App.esLiderOColiderDeMesa` en `app.js`).
