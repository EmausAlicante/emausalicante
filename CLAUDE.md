...
- `index.html` + `store.js` + `app.js` — la app de gestión, exige sesión (login por enlace mágico). Vistas: Panel, Contactos, Equipo de zona, Retiros (con Documentos y Cartas dentro del detalle), Actividades, Material (inventario de ropa), Formulario público (solo genera/copia el enlace, no es funcional aquí), Ajustes.
